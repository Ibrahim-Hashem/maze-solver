...
class Maze:
    def __init__(self, x1, y1, num_rows, num_cols, cell_size_x, cell_size_y, win=None, seed=None):
        ...
        self._win = win
        self._seed = random.randint(0,seed) if seed is not None else None
    ...
    def _solve_r(self, i, j):
        ...
        curr = self._cells[i][j]
        curr._visited = True
        if i > 0 and not self._cells[i-1][j]._visited and not curr.has_left_wall:
            if self._solve_r(i-1, j):
                curr.draw_move(self._cells[i-1][j])
                return True
        if j > 0 and not self._cells[i][j-1]._visited and not curr.has_top_wall:
            if self._solve_r(i, j-1):
                curr.draw_move(self._cells[i][j-1])
                return True
        if i < self._num_cols-1 and not self._cells[i+1][j]._visited and not curr.has_right_wall:
            if self._solve_r(i+1, j):
                curr.draw_move(self._cells[i+1][j])
                return True
        if j < self._num_rows-1 and not self._cells[i][j+1]._visited and not curr.has_bottom_wall:
            if self._solve_r(i, j+1):
                curr.draw_move(self._cells[i][j+1])
                return True
        return False